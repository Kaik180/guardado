...
public class Normal {
    ...
    public static int[] quitarNumeros(int[] vector) {
        int indicePaso = 1; // empezamos en el segundo elemento (posición 1), cuyo valor es 3
        while (indicePaso < vector.length) {
            int paso = vector[indicePaso];
            if (paso <= 1) break; // seguridad, aunque no debería ocurrir

            vector = eliminarCadaPaso(vector, paso);
            indicePaso++; // avanzamos al siguiente paso dentro del nuevo vector
        }
        return vector;
    }
